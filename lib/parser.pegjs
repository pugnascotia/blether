...
classNameComponent = first:[A-Z] others:[a-zA-Z0-9]* {return first + others.join("");}

classNameList = "." className:classNameComponent { return className; }

className = first:classNameComponent others:classNameList* { return [first].concat(others).join("."); }
...
operand = literal / className:className { return new Blether.ClassName(className).at(line(), column(), text()); }
...
classAndMethod = "!" className:className ws classMethod:("class" separator+)? body:method "!" ws? "." {
